...
# pragma region AABB class
	...
	AABB::AABB(const glm::vec3& min, const glm::vec3& max, Logger *logger) : min(min), max(max)
	{
		this->logger = logger;
	}
	...
	bool AABB::intersects(const BoundingBox& other) const {
		if (other.getType() == Type::AABB) {
			...
			logger->error("AABB::intersects: Intersection with other bounding box types not implemented yet.");
		return false;
	}
